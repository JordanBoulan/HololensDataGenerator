...
Last Modified Date: 03/20/17
Description: There are 7 sets of data that are generated randomly, but form a general pattern by using previous values to generate future values.
...
class PAVDataGenerator:
    ...
    @cvar MIN_GFORCE_ANGLE: The minimum possible polar coordinate angle for gforce generator to use
    @cvar MAX_GFORCE_ANGLE: The maximum possible polar coordinate angle for gforce generator to use
    @cvar MIN_GFORCE_RADIUS: The minimum possible polar coordinate radius for gforce generator to use
    @cvar MAX_GFORCE_RADIUS: The maximum possible polar coordinate radius for gforce generator to use
    @cvar NEXT_VALUE_RANGE_ANGLE all values after the first one will be within +- NEXT_VALUE_RANGE_ANGLE of the previous value
    @cvar NEXT_VALUE_RANGE_RADIUS all values after the first one will be within +- NEXT_VALUE_RANGE_RADIUS of the previous value



    @group Storage: setCount, packetNumber, batteryLevel, altitudeLevel, headingDirection, airspeedLevel, temperatureLevel, fanRPM, gforce
    ...
    @ivar gforce: The current set of gforce data tuples
    ...
    MIN_GFORCE_ANGLE = 0
    MAX_GFORCE_ANGLE = 360
    MIN_GFORCE_RADIUS = 0
    MAX_GFORCE_RADIUS = 1
    NEXT_VALUE_RANGE_ANGLE = 20
    NEXT_VALUE_RANGE_RADIUS = 0.1
    ...
    def batteryDataGeneration(self):
        """
        Returns a set of random set of battery data, using previous values to generate next ones
        """
    ...
    def altitudeDataGeneration(self):
        """
        Returns a set of random altitudes, using previous values to generate next ones
        """
    ...
    def headingDataGeneration(self):
        """
        Returns a set of random headings in degree's, using previous values to generate next ones
        """
    ...
    def temperatureDataGeneration(self):
        """
        Returns a set of temperature, using previous values to generate next ones
        """
    ...
    def fanRPMDataGeneration(self):
        """
        Returns a set of fan, using previous values to generate next ones
        """
    ...
    def airspeedDataGeneration(self):
        """
        Returns a set of random airspeed data, using previous values to generate next ones
        """
    ...
    def gforceDataGeneration(self):
        """
        Returns a set of random gforce angle and radius data, using previous values to generate the next ones
        """

        gforceData = []  # Set to be returned
        firstAngleVal = random.uniform(self.MIN_GFORCE_ANGLE, self.MAX_GFORCE_ANGLE)
        lowerAngleVal = firstAngleVal - self.NEXT_VALUE_RANGE_ANGLE
        upperAngleVal = firstAngleVal + self.NEXT_VALUE_RANGE_ANGLE
        firstRadiusVal = random.uniform(self.MIN_GFORCE_RADIUS, self.MAX_GFORCE_RADIUS)
        lowerRadiusVal = firstRadiusVal - self.NEXT_VALUE_RANGE_RADIUS
        upperRadiusVal = firstRadiusVal + self.NEXT_VALUE_RANGE_RADIUS
        
        while len(gforceData) < self.PACKETS_PER_SET:
            angleVal = random.uniform(lowerAngleVal, upperAngleVal)
            radiusVal = random.uniform(lowerRadiusVal, upperRadiusVal)
            polarGforce = (round(angleVal, self.NUM_DECIMALS), round(radiusVal, self.NUM_DECIMALS))
            gforceData.append(polarGforce)

            upperAngleVal = angleVal + self.NEXT_VALUE_RANGE_ANGLE
            lowerAngleVal = angleVal - self.NEXT_VALUE_RANGE_ANGLE
            if upperAngleVal > self.MAX_GFORCE_ANGLE:
                upperAngleVal = self.MAX_GFORCE_ANGLE
            if lowerAngleVal < self.MIN_GFORCE_ANGLE:
                lowerAngleVal = self.MIN_GFORCE_ANGLE

            upperRadiusVal = radiusVal + self.NEXT_VALUE_RANGE_RADIUS
            lowerRadiusVal = radiusVal - self.NEXT_VALUE_RANGE_RADIUS
            if upperRadiusVal > self.MAX_GFORCE_RADIUS:
                upperRadiusVal = self.MAX_GFORCE_RADIUS
            if lowerRadiusVal < self.MIN_GFORCE_RADIUS:
                lowerRadiusVal = self.MIN_GFORCE_RADIUS
    ...
    def PAVDataCollection(self):
        ...
        if self.setCount >= self.PACKETS_PER_SET or self.setCount == 0:
            ...
            self.gforce = self.gforceDataGeneration()
...
class GenerateAndBroadcast:
    ...
    def jsonConversion(self, PAVGeneratedData):
        ...
        dataPacket = self.PAVDataStructure(
            ...
            PAVGeneratedData[6], # gforce
            PAVGeneratedData[7], # packet number
            )
